...
SCRIPT_UNDER_TEST="${SCRIPT_DIR}/create_function_resource.sh"
...
jq -e '
  ...
  .time == "2026-05-09T00:00:00Z" and
  .datacontenttype == "application/json" and
  .data.resource_id == "11111111-1111-4111-8111-111111111111" and
  .data.workspace_id == "workspace-9" and
  .data.function_key == "billing" and
  .data.display_name == "Fixture Function Resource" and
  .data.resource_type == "document" and
  .data.resource_tags == ["section_1"]
' "${TMP_DIR}/custom.stdin" >/dev/null

if ! grep -q "resource_id=11111111-1111-4111-8111-111111111111" "${TMP_DIR}/custom.stdout"; then
  printf 'stdout does not include resource_id\n' >&2
  exit 1
fi
...
jq -e --arg uuid_pattern "${uuid_pattern}" '
  ...
  .subject == .data.resource_id and
  .data.workspace_id == "workspace-1" and
  .data.function_key == "todo"
...
first_resource_id="$(jq -r '.data.resource_id' "${TMP_DIR}/default_first.stdin")"
second_resource_id="$(jq -r '.data.resource_id' "${TMP_DIR}/default_second.stdin")"
if [[ "${first_resource_id}" == "${second_resource_id}" ]]; then
  printf 'resource IDs should differ between runs\n' >&2
  exit 1
fi
