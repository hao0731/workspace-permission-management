...
type fakeUpsertPublisher struct {
	events []resource.ResourceUpsertEvent
	err    error
}

func (f *fakeUpsertPublisher) PublishResourceUpsert(_ context.Context, event resource.ResourceUpsertEvent) error {
...
func TestResourceServiceHandleCreateCommandPublishesUpsert(t *testing.T) {
	...
	err := service.HandleResourceCreate(context.Background(), resource.ResourceCreateCommand{
